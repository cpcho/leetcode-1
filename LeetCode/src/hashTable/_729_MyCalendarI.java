 ...
 * takes every time less than 20, but not including 20. Note:
 * 
 * The number of calls to MyCalendar.book per test case will be at most 1000. In
 * calls to MyCalendar.book(start, end), start and end are integers in the range
 * [0, 10^9].
 *
...
public class _729_MyCalendarI {
	...
	public boolean book(int start, int end) {
		...
		if ((pre == null || start >= map.get(pre)) && (next == null || next >= end)) {
			map.put(start, end);
			return true;
		}
		return false;
